...
fun MainScreen(onItemClick: (NavKey) -> Unit, modifier: Modifier = Modifier) {
    ...
    val viewModel: MainScreenViewModel = viewModel {
        MainScreenViewModel(
                ...
                alertSettingsRepository = AlertSettingsRepository(context.applicationContext)
        )
    }
    ...
    Column(modifier = modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        Text(text = "Popis Alerta", style = MaterialTheme.typography.headlineMedium)
    ...
    if (showClearVisitsDialog) {
        AlertDialog(
                ...
                title = { Text(text = "¿Borrar historial?") },
                text = {
                    Text(
                            text =
                ...
                dismissButton = {
                    Button(onClick = { showClearVisitsDialog = false }) { Text(text = "Cancelar") }
                }
        )
    }
}
...
private fun BathroomVisitRow(visit: BathroomVisitEntity) {
    val date = DateFormat.getMediumDateFormat(LocalContext.current).format(Date(visit.startedAt))
    val time = DateFormat.getTimeFormat(LocalContext.current).format(Date(visit.startedAt))
    ...
    Row(
        ...
        Column {
            Text(text = "Visita al baño", style = MaterialTheme.typography.titleMedium)
...
private fun MainScreenEmptyPreview() {
    PopisAlertaTheme {
        Column(
            ...
            Text(text = "Popis Alerta", style = MaterialTheme.typography.headlineMedium)
            Text(text = "Avisos activos", style = MaterialTheme.typography.titleMedium)
            Text(text = "Visitas al baño", style = MaterialTheme.typography.titleLarge)
            Text(
                    text = "Todavía no hay visitas registradas.",
                    style = MaterialTheme.typography.bodyLarge
...
private fun MainScreenWithVisitsPreview() {
    PopisAlertaTheme {
        Column(
            ...
            Text(text = "Visitas al baño", style = MaterialTheme.typography.titleLarge)
            BathroomVisitRow(
                    BathroomVisitEntity(id = 1, startedAt = 1_770_000_000_000, notified = false)
            )
        }
    }
}
